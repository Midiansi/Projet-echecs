...
def dessiner_piece(tab):
    ...
    global cheque
    #On dessine aussi un avertissement pour les rois
...
def cheque_test(tab: list) -> list:
    """Renvoie un tableau avec les cases du mouvement d'une piece qui peut manger un roi"""
    global echiquier
            ...
            if (piece != 0):
                ...
                for test in test_piece:
                    if echiquier[test[0]][test[1]] != 0\
                       and echiquier[test[0]][test[1]][0] == "6"\
                       and echiquier[test[0]][test[1]][1] != couleur_piece:
...
def update():
    global case_gauche, tab_case_droite, commencer, compteur_mouv, vainqueur, echiquier, jeu, en_passant,\
           mouvement_precedent, dames_manger, nb_pieceB, nb_pieceR, roque, prom_coord, prom_couleur, cheque
...
def draw():     
    if commencer == 0: #menu de selection
        ...
        if case_gauche != "": #Clic gauche, case bleu clair
            pyxel.rect(case_gauche[1] * 16, case_gauche[0] * 16, 16, 16, 5)
        for case_cheque in cheque: #Case du roi quand il y a cheque, case bleu fonce
            pyxel.rect(case_cheque[1] * 16, case_cheque[0] * 16, 16, 16, 1)
        for case_droite in tab_case_droite: #Case mouvement possible, vert
            if case_droite[0] % 2 == 0:
                if case_droite[1] % 2 == 0:
                    pyxel.rect(case_droite[1] * 16, case_droite[0] * 16, 16, 16, 11)
                else:
                    pyxel.rect(case_droite[1] * 16, case_droite[0] * 16, 16, 16, 3)
            else:
                if case_droite[1] % 2 == 0:
                    pyxel.rect(case_droite[1] * 16, case_droite[0] * 16, 16, 16, 3)
                else:
        ...
        pyxel.text(40, 80, f"En {compteur_mouv} mouvements", 0)
