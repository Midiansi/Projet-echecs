...
def valide(clic: tuple, tab: list) -> list:
    ...
    if piece == "1":
        tab =  [(clic[0]-1,clic[1]), (clic[0]-1,clic[1]-1), (clic[0]-1,clic[1]+1),
               (clic[0]+1,clic[1]), (clic[0]+1,clic[1]-1), (clic[0]+1,clic[1]+1)]
...
def update():
    ...
    if commencer == 1:
        ...
        if pyxel.btnp(pyxel.MOUSE_BUTTON_RIGHT):
            clic_temp = clic_case()
            for case in tab_case_droite:
                if clic_temp == case:
                    case = echiquier[case_gauche[0]][case_gauche[1]]
                    echiquier[clic_temp[0]][clic_temp[1]] = case
