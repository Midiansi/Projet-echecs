...
def valide(clic: tuple, tab: list) -> list:
    ...
    if piece == "7":
        ...
        gauche_manger = (clic[0]+(2*direction),clic[1]-2)
        if encadre(gauche_manger) and (echiquier[gauche[0]][gauche[1]] != 0)\
           and (echiquier[gauche[0]][gauche[1]][1] != couleur) and\
           echiquier[gauche_manger[0]][gauche_manger[1]] == 0:
            tab.append((gauche_manger[0],gauche_manger[1]))
            dames_manger += [(gauche_manger),(gauche)]
                
        droite = (clic[0]+(1*direction),clic[1]+1)
        if encadre(droite) and (echiquier[droite[0]][droite[1]] == 0):
            tab.append((droite[0],droite[1]))
        
        droite_manger = (clic[0]+(2*direction),clic[1]+2)
        if encadre(droite_manger) and (echiquier[droite[0]][droite[1]] != 0)\
           and (echiquier[droite[0]][droite[1]][1] != couleur) and\
           echiquier[droite_manger[0]][droite_manger[1]] == 0:
            tab.append((droite_manger[0],droite_manger[1]))
            dames_manger += [(droite_manger),(droite)]
        
        
    #coordonnes valides pour une damme:
    if piece == "8":
        for i in range(0,8):
            tab.append((clic[0]+1+i,clic[1]+1+i))
            if not encadre((clic[0]+1+i,clic[1]+1+i)) or echiquier[clic[0]+1+i][clic[1]+1+i] != 0:
        ...
        if pyxel.btnp(pyxel.MOUSE_BUTTON_RIGHT):
            ...
            for case_test in tab_case_droite:
                if clic_temp == case_test:
                    ...
                    if case[0] == "7" or case[0] == "8":
                        for i in range(0, len(dames_manger), 2):
                            if clic_temp == dames_manger[i]:
